...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  // Play the hovered card's video (nothing plays at rest — keeps it light).
  const playCardVideo = (e: MouseEvent<HTMLElement>) => {
    e.currentTarget.querySelector("video")?.play().catch(() => {});
  };
  const pauseCardVideo = (e: MouseEvent<HTMLElement>) => {
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const media = (
              <div className="relative w-full h-full overflow-hidden">
                ...
                {(card.title || card.subtitle) && (
                  <div className="absolute inset-0 z-20 flex flex-col justify-end bg-gradient-to-t from-black/85 via-black/30 to-transparent p-4 text-start">
                    {card.title && (
                    ...
                    {card.subtitle && (
                      <p className="mt-1 text-xs leading-snug text-white/80">{card.subtitle}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer" onMouseEnter={playCardVideo} onMouseLeave={pauseCardVideo}>{media}</a>
            ) : (
              <div key={index} className="fan-card" onMouseEnter={playCardVideo} onMouseLeave={pauseCardVideo}>{media}</div>
            );
          })}
        </div>
      </div>
